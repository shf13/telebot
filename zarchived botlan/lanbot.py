...
def _format_prayer_message(payload: dict, lang: str) -> str:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{tr(lang, 'intro_short')} {tr(lang, 'menu_prompt')}",
...
async def language_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /Language — allow changing language any time.
    """
    storage: Storage = context.application.bot_data["storage"]
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id
    ...
    # If already set, show prompt in their language; else show the 3-language prompt
    if lang:
...
async def settime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            tr(lang, "settime_usage"),
            reply_markup=_preset_time_kb(lang),
        )
        return
...
async def now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = _format_prayer_message(payload, lang)
    await update.message.reply_text(
        msg,
        parse_mode="HTML",
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("LANG:"):
        ...
        # Auto-show today's times once after choosing language
        payload, friendly = _load_today_or_friendly(data_file, lang)
        if friendly:
