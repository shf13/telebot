...
from quran import QuranProvider  # NEW IMPORT
...
def _format_prayer_message(payload: dict, lang: str, ayah: dict | None = None) -> str:
    """
    Format the prayer message.
    If 'ayah' is provided, it expects a dict with keys: 'ar', 'en', 'ru', 'ref'.
    """
    ...
    if ayah:
        ...
        # Build text based on language rules
        ayah_content = []
        
        # Arabic text is always shown (based on your request description implied)
        # or as base. Let's follow specific rules:
        # Ar: Ar only
        # En: En + Ar
        # Ru: Ar + Ru (Assuming users want to read the Arabic source + translation)
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("LANG:"):
        ...
        # Fetch Ayah for immediate display on lang change
        ayah = quran.get_random_ayah()
        msg = _format_prayer_message(payload, lang, ayah=ayah)
